...
def calcular_criticidad(var_porcentual):
    """Evalúa la criticidad basada en el % de variación respecto a la media."""
    if var_porcentual == '---' or pd.isnull(var_porcentual): 
        return '---'
    try:
        ...
        else: return 'REVISAR' # Caídas bruscas (ej. sensor desconectado)
    except ValueError: 
        return '---'
...
def procesar_unidad(df_in, month_order, latest_month, group_keys, unidad):
    ...
    # NUEVO: Cálculo de Variación Porcentual
    ...
    # Evaluamos criticidad con el nuevo porcentaje universal
...
if uploaded_file is not None:
    if st.button("Procesar y Generar Dashboard", type="primary"):
        with st.spinner('Analizando datos, calculando variaciones relativas y dibujando gráficos...'):
            try:
                ...
                for row_str in lines:
                    ...
                    # Soporte ampliado para prefijos PP, PF, PC, PR
                ...
                if df_data.empty: st.error("No se encontraron datos válidos.")
                    ...
                    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                        ...
                        def escribir_lista_negra(df, fila):
                            ...
                            # Convertimos a float para ordenar correctamente ignorando los '---'
                            ...
                            columnas_mostrar = ['Area', 'Equipment', 'Unit', 'Criticality', 'Var % vs Avg', 'Latest vs Avg']
                            for col_num, col_name in enumerate(columnas_mostrar):
                                ws_dash.write(fila, col_num, col_name, fmt_header)
                            fila += 1
                            
                            for _, row in df_alertas.iterrows():
                                for i, col in enumerate(['Area', 'Equipment', 'Unit']): 
                                    ws_dash.write(fila, i, row[col], fmt_border)
                                ws_dash.write(fila, 3, row['Criticality'], formato_alto if row['Criticality'] == 'ALTO' else formato_media)
                                
                                # Añadimos el símbolo % visualmente al valor de variación
                                val_var = f"{row['Var % vs Avg']}%" if row['Var % vs Avg'] != '---' else '---'
                                ws_dash.write(fila, 4, val_var, fmt_border)
